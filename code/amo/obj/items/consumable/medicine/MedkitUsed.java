...
public class MedkitUsed extends Item implements Usable {
    ...
    @Override
    public void use() {
        util.TextUtils.blueText(getHolder().tryToGetRealName() + " открыл(-а) " + getName() + " и начал(-а) копошиться внутри. Найдя нужные медицинские средства, он(-а) обработал(-а) свои раны и выпил(-а) пару таблеток. Ничегошеньки в аптечке больше не осталось, поэтому он(-а) выбросил(-а) пустую аптечку куда подальше.");
        getHolder().heal(20);
        destroy();
    }
}
